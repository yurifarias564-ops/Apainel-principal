...
def get_painel_html(user_cfg, cache_data=None, is_admin_view=False, target_user=None):
    ...
    head_script = (
        ...
        "        var ls=document.getElementById('loadingScreen');\n"
        "        if(ls)ls.style.display='none';\n"
        "        // Se ainda esta atualizando, busca novamente em background\n"
        "        if(j.status==='atualizando'){setTimeout(_crm_fetch,8000);}\n"
        "      } else if(j&&(j.status==='erro'||j.erro)){\n"
        "        // Mostra erro no lugar do loading\n"
        "        var ls=document.getElementById('loadingScreen');\n"
        "        var le=document.getElementById('loadErr');\n"
        "        if(le){le.style.display='block';le.innerHTML='&#10060; Erro ao conectar ao Kommo:<br><b>'+(j.erro||'verifique as credenciais')+'</b><br><small style=\"font-size:11px;opacity:.7\">Tentando novamente em 30s...</small>';}\n"
        "        if(ls){ls.querySelector&&ls.querySelector('.spin')&&(ls.querySelector('.spin').style.display='none');}\n"
        ...
        "        var le=document.getElementById('loadErr');\n"
        "        if(le){le.style.display='block';le.innerHTML='&#9203; Tempo esgotado ao carregar dados. Recarregue a pagina.';}\n"
        "      }\n"
...
"""
    ...
    def _handle_GET(self):
        ...
        if path == "/admin/meu-painel":
            ...
            # Embute dados do cache no HTML — abre instantaneo se ja tem cache
        ...
        # API dados — retorna cache imediatamente se disponivel, senao long-poll ate 20s
        if path == "/api/dados":
            if not sess:
            ...
            # Se ja tem dados em cache, retorna imediatamente
            if c_dados is not None:
                with lock:
                    resp = {"status": cache["status"], "atualizado_em": cache["atualizado_em"],
                            "erro": cache["erro"], "dados": cache["dados"]}
                return self.send_json(200, resp)
            # Sem dados ainda — aguarda ate 20s pelo primeiro fetch
            for _ in range(40):
                time.sleep(0.5)
                with lock:
                    c_dados  = cache["dados"]
...
if __name__ == "__main__":
    ...
    # Inicia fetch do painel admin principal no startup
    if CFG.get("subdomain") and CFG.get("token"):
        threading.Thread(
            target=buscar_kommo_user,
            args=("__admin__", CFG["subdomain"], CFG["token"]),
