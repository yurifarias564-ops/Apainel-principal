...
# ─── USERS / CLIENTES ─────────────────────────────────────────────────────────
# Cache em memória dos usuários (fallback quando disco é somente leitura)
_users_memory = {}

def load_users():
    """Carrega users.json do disco ou retorna cache em memória."""
    if USERS_FILE.exists():
        try: 
            data = json.loads(USERS_FILE.read_text(encoding="utf-8"))
            _users_memory.clear()
            _users_memory.update(data)
            return dict(data)
        except: pass
    return dict(_users_memory)

def save_users(users):
    """Salva users.json no disco; se não conseguir, mantém em memória."""
    _users_memory.clear()
    _users_memory.update(users)
    try:
        USERS_FILE.write_text(json.dumps(users, ensure_ascii=False, indent=2), encoding="utf-8")
    except Exception as e:
        print(f"  [!] Disco somente leitura, usando memoria: {e}")
