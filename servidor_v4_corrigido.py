...
def checar_alertas_email():  # utilitário — não chamado automaticamente, pode ser chamado manualmente
    ...
    try:
        ...
        for uname in usuarios:
            ...
            hoje_ini = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0).timestamp()
            ...
            if inativos:
                chave = f"{prefixo}inativos_{datetime.now().strftime('%Y%m%d')}"
                if _pode_alertar(chave):
...
def auto_alertas_email():
    """Thread de emails automáticos: 06:00h (manhã), 19:00h (semana), alertas a cada hora."""
    ...
    while True:
        ...
        # Alertas em tempo real — a cada hora
        if agora.hour != _ultima_hora_alerta:
            checar_alertas_email()
            _ultima_hora_alerta = agora.hour
