...
VENDAS: {stats.get('ganhos',0)} | PERDAS: {stats.get('perdidos',0)} | EM ANDAMENTO: {stats.get('em_andamento',0)}
TOTAL LEADS: {stats.get('total_leads',0)} | SEM FOLLOW-UP: {stats.get('leads_sem_fup',0)} | PARADOS +7 DIAS: {stats.get('leads_parados',0)}
CONVERSÃO GERAL: {stats.get('taxa_conversao',0)}%
CANAIS: {canal_txt}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle_GET(self):
        ...
        if path == "/api/raiox":
            ...
            qs = parse_qs(parsed.query)
            target_qs  = qs.get("user",      [None])[0]
            filtro_vend = qs.get("vendedor",  [None])[0]  # filtro por tag/vendedor
            if sess["is_admin"] and target_qs:
                cache_key = target_qs
            elif sess["is_admin"]:
                cache_key = "__admin__"
            ...
            # Filtrar por vendedor/tag se solicitado
            if filtro_vend:
                fv = filtro_vend.upper().strip()
                filtrado = [
                    l for l in leads_raw
                    if fv in (l.get("vendedor") or "").upper()
                    or fv in (l.get("tags") or "").upper()
                ]
                if filtrado:
                    dados_cache = dict(dados_cache)
                    dados_cache["leads"] = filtrado
                else:
                    return self.send_json(200, {"erro": "sem_dados", "msg": f"Nenhum lead encontrado para o vendedor/tag '{filtro_vend}'."})
            meta_val  = dados_cache.get("meta_mes") or CFG["meta_mes_padrao"]
            mes_atual = datetime.now().strftime("%Y-%m")
            stats = compute_raiox_stats(dados_cache, mes_atual, meta=meta_val)
            if filtro_vend:
                stats["filtro_vendedor"] = filtro_vend
            if not ANTHROPIC_KEY:
