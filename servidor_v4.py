...
# ══════════════════  FOLLOW-UP POR SESSÃO WHATSAPP  ═══════════════════════

def buscar_eventos_chat(base_url, token, desde_dias=60):
    """Busca eventos de mensagem WhatsApp dos últimos N dias via Kommo /events."""
    desde_ts = int(time.time()) - desde_dias * 86400
    tipos    = "incoming_chat_message,outgoing_chat_message"
    url      = (f"{base_url}/events?filter[type]={tipos}"
                f"&filter[created_at][from]={desde_ts}&limit=250")
    eventos  = []
    page     = 1
    while True:
        try:
            data = kommo_get(url + f"&page={page}", token)
            if not data: break
            items = (data.get("_embedded") or {}).get("events") or []
            for ev in items:
                et = (ev.get("entity_type") or "").lower()
                if et not in ("lead", "leads"): continue
                eventos.append({
                    "lead_id":    str(ev.get("entity_id", "")),
                    "ts":         ev.get("created_at", 0),
                    "tipo":       ev.get("type", ""),
                    "criado_por": ev.get("created_by", 0),
                })
            if not (data.get("_links") or {}).get("next"):
                break
            page += 1
        except Exception as e:
            print(f"  [FU] erro pg{page}: {e}", flush=True)
            break
    print(f"  [FU] {len(eventos)} eventos encontrados (últimos {desde_dias}d)", flush=True)
    return eventos


def calcular_sessoes_fu(eventos, gap_horas=24):
    """Agrupa eventos por lead em sessões separadas por gap > gap_horas."""
    por_lead = {}
    for ev in eventos:
        lid = ev["lead_id"]
        if not lid: continue
        por_lead.setdefault(lid, []).append(ev["ts"])
    resultado = {}
    gap_seg   = gap_horas * 3600
    for lid, tss in por_lead.items():
        tss_sorted = sorted(tss)
        sessoes    = []
        inicio = fim = tss_sorted[0]
        for ts in tss_sorted[1:]:
            if ts - fim > gap_seg:
                sessoes.append({"inicio": inicio, "fim": fim})
                inicio = ts
            fim = ts
        sessoes.append({"inicio": inicio, "fim": fim})
        resultado[lid] = {
            "sessoes":          len(sessoes),
            "ultimo_ts":        tss_sorted[-1],
            "sessoes_detalhes": sessoes,
        }
    return resultado


def calcular_fu_por_vendedor(sessoes_por_lead, leads):
    """Agrega métricas de FU por vendedor para leads em andamento."""
    agora    = int(time.time())
    vend_map = {}
    for l in leads:
        vend = l.get("vendedor") or "Nao atribuido"
        lid  = str(l.get("id", ""))
        if vend not in vend_map:
            vend_map[vend] = {
                "total_leads": 0, "leads_com_fu": 0, "leads_sem_fu": 0,
                "sessoes_total": 0, "ultimo_contato_ts": None,
                "dist_sessoes": {},
            }
        vm = vend_map[vend]
        vm["total_leads"] += 1
        fu = sessoes_por_lead.get(lid)
        if fu and fu["sessoes"] > 0:
            vm["leads_com_fu"]  += 1
            vm["sessoes_total"] += fu["sessoes"]
            n = str(min(fu["sessoes"], 5))
            vm["dist_sessoes"][n] = vm["dist_sessoes"].get(n, 0) + 1
            uts = fu["ultimo_ts"]
            if vm["ultimo_contato_ts"] is None or uts > vm["ultimo_contato_ts"]:
                vm["ultimo_contato_ts"] = uts
        else:
            vm["leads_sem_fu"] += 1
    out = {}
    for vend, vm in vend_map.items():
        total = vm["total_leads"]
        taxa  = round(vm["leads_com_fu"] / total * 100, 1) if total else 0
        uts   = vm["ultimo_contato_ts"]
        dias  = round((agora - uts) / 86400, 1) if uts else None
        out[vend] = {
            "total_leads":       total,
            "leads_com_fu":      vm["leads_com_fu"],
            "leads_sem_fu":      vm["leads_sem_fu"],
            "sessoes_total":     vm["sessoes_total"],
            "ultimo_contato_ts": uts,
            "dias_sem_contato":  dias,
            "taxa_fu_pct":       taxa,
            "dist_sessoes":      vm["dist_sessoes"],
        }
    return out
...
def _buscar_kommo_user_inner(username, subdomain, token):
    ...
    try:
        ...
        try:
            leads_and   = [l for l in all_leads
                           if (status_map.get(l.get("etapa")) or {}).get("tipo") == "andamento"]
            print(f"  [FU] iniciando cálculo — {len(leads_and)} leads em andamento", flush=True)
            ev_chat     = buscar_eventos_chat(base_url, token, desde_dias=60)
            fu_sessoes  = calcular_sessoes_fu(ev_chat, gap_horas=24)
            fu_por_vend = calcular_fu_por_vendedor(fu_sessoes, leads_and)
            print(f"  [FU] vendedores calculados: {list(fu_por_vend.keys())}", flush=True)
        except Exception as e:
            import traceback
            print(f"  [FU] erro: {e}", flush=True)
            print(traceback.format_exc(), flush=True)
            fu_sessoes = {}; fu_por_vend = {}
        ...
        resultado = {
            ...
            "fu_sessoes":      fu_sessoes,
            "fu_por_vendedor": fu_por_vend,
        }
        with lock:
...
if __name__ == "__main__":
    ...
    print("=" * 52)
    print("  Admin : " + CFG['admin_user'] + " / " + CFG['admin_pass'])
