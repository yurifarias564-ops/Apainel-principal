...
def load_config():
    ...
    return {
        ...
        # Horários de email (hora local após TZ_OFFSET)
        "hora_manha":      int(os.environ.get("HORA_MANHA", 6)),   # relatório manhã (padrão 06:00)
        "hora_noite":      int(os.environ.get("HORA_NOITE", 19)),   # relatório noite  (padrão 19:00)
        # Horas em que alertas são verificados (lista separada por vírgula, ex: "8,12,18")
        "horas_alerta":    [int(h) for h in os.environ.get("HORAS_ALERTA", "8,12,18").split(",") if h.strip().isdigit()],
        # Offset de fuso horário em relação ao UTC (Brasil = -3)
...
def hora_local():
    """Retorna datetime ajustado ao fuso configurado em TZ_OFFSET (para servidores em UTC)."""
...
ALERT_EMAIL = os.environ.get("ALERT_EMAIL", "")  # email que recebe os alertas (pode ser o mesmo do Gmail)
...
def _disk_cache_path(username):
    safe = re.sub(r"[^\w\-]", "_", username)
    return DATA_DIR / f"cache_{safe}.json"

def save_cache_disk(username, cache_entry):
    """Salva cache em disco para sobreviver restart do servidor."""
    try:
        path = _disk_cache_path(username)
        DATA_DIR.mkdir(parents=True, exist_ok=True)
        tmp = path.with_suffix(".tmp")
        tmp.write_text(
            json.dumps({"dados": cache_entry["dados"],
                        "atualizado_em": cache_entry["atualizado_em"]},
                       ensure_ascii=False, separators=(",", ":")),
            encoding="utf-8"
        )
        tmp.replace(path)
    except Exception as e:
        print(f"  [cache-disk] Erro ao salvar {username}: {e}")

def load_cache_disk(username):
    """Carrega cache do disco (dados do ultimo fetch antes do restart)."""
    try:
        path = _disk_cache_path(username)
        ...
        print(f"  [cache-disk] Erro ao ler {username}: {e}")
    return None

def get_cache(username):
    with _caches_init_lock:
        if username not in _caches:
            disk = load_cache_disk(username)
            if disk:
                print(f"  [cache-disk] Restaurado '{username}' ({disk.get('atualizado_em','?')})")
                _caches[username] = {
                    "dados": disk["dados"],
                    "atualizado_em": disk["atualizado_em"],
                    "status": "ok",
                    "erro": None,
                }
            else:
...
def remove_cache(username):
    ...
    try:
        _disk_cache_path(username).unlink(missing_ok=True)
    except: pass
    ...
    for t in range(tentativas):
        try:
            ...
            if e.code in (429, 503) and t < tentativas - 1:
                time.sleep(min(2**t, 60)); continue  # cap em 60s
...
def fetch_all(base_url, token, endpoint, emb_key, extra=""):
    ...
    while True:
        ...
        page += 1
        time.sleep(0.05)  # rate-limit suave: 50ms por pagina (era 120ms)
    return items
    ...
    try:
        ...
        for p in pipelines:
            ...
            for i, s in enumerate(statuses):
                ...
                status_map[s["id"]] = {"label": s["name"], "funil": p["name"], "tipo": tipo, "order": i}

        users    = fetch_all(base_url, token, "/users", "users")
        user_map = {u["id"]: u.get("name") or f"User {u['id']}" for u in users}

        # Sync incremental: so busca leads modificados desde a ultima sync
        cached_leads_map = {}
        last_sync_ts     = None
        with lock:
        ...
        if prev and prev.get("leads"):
            ...
            last_sync_ts = prev.get("_last_kommo_ts")
        ...
        print(f"  [contatos] {len(ids)} unicos -> {len(batches)} lotes x {WORKERS} workers")
        ...
        if cached_leads_map:
            for l in all_leads:
                cached_leads_map[l["id"]] = l
            all_leads = list(cached_leads_map.values())
            print(f"  [merge] total apos merge: {len(all_leads)} leads")
        ...
        with lock:
            ...
            cache["status"] = "ok"
            cache["erro"]   = None
        # Persiste em disco para sobreviver restart
        threading.Thread(
            target=save_cache_disk,
            args=(username, {"dados": resultado, "atualizado_em": cache["atualizado_em"]}),
            daemon=True
        ).start()
...
def auto_raiox_mensal():
    ...
    while True:
        ...
        dia_disparo  = dias_no_mes  # último dia do mês (28, 29, 30 ou 31)
...
# Log de emails enviados (últimos 300)
_email_log      = []
...
def checar_alertas_email():  # utilitário — não chamado automaticamente, pode ser chamado manualmente
    ...
    try:
        ...
        for uname in usuarios:
            ...
            _agora_local = hora_local()
            hoje_ini = _agora_local.replace(hour=0, minute=0, second=0, microsecond=0).timestamp()
            ...
            if inativos:
                chave = f"{prefixo}inativos_{_agora_local.strftime('%Y%m%d')}"
                if _pode_alertar(chave):
            ...
            if meta > 0:
                ...
                valor_ganho = sum((l.get("valor") or 0) for l in leads
                ...
                pct_meta = valor_ganho / meta
                if pct_mes > 0.4 and pct_meta < 0.3:
                    chave = f"{prefixo}meta_{hoje.year}_{hoje.month}"
                    if _pode_alertar(chave):
                        itens = [
                            f"<b>R$ {valor_ganho:,.0f}</b> de <b>R$ {meta:,.0f}</b> ({round(pct_meta*100)}% da meta)",
...
def enviar_relatorio_manha(only_user=None):
    ...
    try:
        ...
        for uname in usuarios:
            ...
            ontem       = hora_local().replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(days=1)
...
def enviar_relatorio_noite(only_user=None):
    ...
    try:
        ...
        for uname in usuarios:
            ...
            hoje       = hora_local().replace(hour=0, minute=0, second=0, microsecond=0)
            ...
            try:
                ...
                dias_mes = 30
            pct_mes    = hoje.day / dias_mes
            ganhos_mes_count = sum(1 for l in leads
                             if status_map.get(l.get("etapa"), {}).get("tipo") == "ganho"
                             and datetime.fromtimestamp(l.get("fechado_em") or l.get("data") or 0).month == hoje.month
                             and datetime.fromtimestamp(l.get("fechado_em") or l.get("data") or 0).year  == hoje.year)
            ganhos_mes = sum((l.get("valor") or 0) for l in leads
                             if status_map.get(l.get("etapa"), {}).get("tipo") == "ganho"
                             and datetime.fromtimestamp(l.get("fechado_em") or l.get("data") or 0).month == hoje.month
                             and datetime.fromtimestamp(l.get("fechado_em") or l.get("data") or 0).year  == hoje.year)
            ...
            if meta > 0:
                ...
                ritmo      = round(faltam / max(dias_rest, 1))
                secoes.append(("🎯 Meta do Mês", [
                    ("Vendas no mês",         f"{ganhos_mes_count} venda(s) · R$ {ganhos_mes:,.0f} de R$ {meta:,.0f}"),
                    ("% atingida",            f"{emoji_meta} {pct_meta}%"),
                    ("Faltam",                f"R$ {faltam:,.0f} em {dias_rest} dia(s)"),
                    ("Ritmo necessário",      f"R$ {ritmo:,.0f}/dia para bater"),
                ]))
...
def auto_alertas_email():
    """Thread de emails automáticos com horários configuráveis via variáveis de ambiente.
    HORA_MANHA   : hora do relatório da manhã  (padrão 6,  ex: HORA_MANHA=7)
    HORA_NOITE   : hora do relatório da noite  (padrão 19, ex: HORA_NOITE=20)
    HORAS_ALERTA : horas de verificação de alertas (padrão "8,12,18", ex: HORAS_ALERTA=9,13,17)
    TZ_OFFSET    : fuso horário relativo ao UTC (padrão -3 = Brasil, ex: TZ_OFFSET=-3)
    """
    time.sleep(120)  # aguarda 2 min no boot
    _enviado_manha      = None
    _enviado_noite      = None
    _ultima_hora_alerta = -1
    while True:
        agora = hora_local()  # usa fuso horário configurado
        hoje  = agora.date()
        hora  = agora.hour

        # Relatório da manhã — horário configurável (padrão 06:00 local)
        if hora == CFG["hora_manha"] and _enviado_manha != hoje:
            print(f"[RELATORIO] {agora.strftime('%H:%M')} — Enviando relatório da manhã...")
            enviar_relatorio_manha()
            _enviado_manha = hoje

        # Relatório da noite — horário configurável (padrão 19:00 local)
        if hora == CFG["hora_noite"] and _enviado_noite != hoje:
            print(f"[RELATORIO] {agora.strftime('%H:%M')} — Enviando relatório da semana...")
            enviar_relatorio_noite()
            _enviado_noite = hoje

        # Alertas — somente nos horários configurados (padrão 08h, 12h e 18h)
        if hora != _ultima_hora_alerta and hora in CFG["horas_alerta"]:
            print(f"[ALERTA] {agora.strftime('%H:%M')} — Verificando alertas automáticos...")
            checar_alertas_email()
            _ultima_hora_alerta = hora
...
PAINEL_HTML_STABLE = DATA_DIR / "painel_stable.html"   # versão liberada para clientes
_painel_html_cache = None
_painel_html_lock  = threading.Lock()

def get_painel_html_base(preview=False):
    """Lê o HTML do disco.
    preview=True  → sempre o arquivo mais recente (admin testa aqui)
    preview=False → versão liberada para clientes (painel_stable.html)
                    Se ainda não foi liberada nenhuma versão, usa a mais recente como fallback.
    """
    global _painel_html_cache
    with _painel_html_lock:
        if not preview and PAINEL_HTML_STABLE.exists():
            # Clientes recebem a versão estável liberada
            return PAINEL_HTML_STABLE.read_text(encoding="utf-8")
        # Admin (preview) ou sem versão estável ainda → arquivo mais recente
        if not PAINEL_HTML.exists():
            return _painel_html_cache  # fallback se arquivo sumiu
        _painel_html_cache = PAINEL_HTML.read_text(encoding="utf-8")
        return _painel_html_cache

def get_painel_html(user_cfg, cache_data=None, is_admin_view=False, target_user=None, preview=False):
    html = get_painel_html_base(preview=preview)
    ...
    head_script = (
        ...
        "    if(typeof renderTab==='function')renderTab(window._tabAtiva||'hoje',dados);\n"
        ...
        "  var tid=setTimeout(function(){ctrl.abort();},55000);\n"
        "  fetch(window._API_URL,{signal:ctrl.signal,credentials:'include'})\n"
        "    .then(function(r){clearTimeout(tid);return r.json();})\n"
        "    .then(function(j){\n"
        "      if(j&&j.dados){\n"
        "        _crm_render(j.dados);\n"
        "        // Esconde loading mesmo se dados vazios\n"
        "        var ls=document.getElementById('loadingScreen');\n"
        "        if(ls)ls.style.display='none';\n"
        "      } else if(j&&(j.status==='erro'||j.erro)){\n"
        "        var ls=document.getElementById('loadingScreen');\n"
        "        if(ls)ls.style.display='none';\n"
        "        var le=document.getElementById('loadErr');\n"
        "        if(!le){le=document.createElement('div');le.id='loadErr';le.style.cssText='position:fixed;top:20px;left:50%;transform:translateX(-50%);z-index:10000;max-width:94vw;width:480px;text-align:center;padding:18px 22px;background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.4);border-radius:12px;color:#ef4444;font-size:13px;line-height:1.6';document.body.appendChild(le);}\n"
        "        le.style.display='block';\n"
        "        le.innerHTML='&#10060; Erro ao conectar ao Kommo:<br><b>'+(j.erro||'verifique as credenciais')+'</b><br><small style=\"font-size:11px;opacity:.7\">Tentando novamente em 30s...</small>';\n"
        "        if(typeof setPill==='function')setPill('error','Erro Kommo');\n"
        "        setTimeout(_crm_fetch,30000);\n"
        "      } else if(j&&j.status==='atualizando'){\n"
        "        if(typeof setPill==='function')setPill('updating','↻ Carregando dados...');\n"
        "        setTimeout(_crm_fetch,5000);\n"
        "      } else{\n"
        "        setTimeout(_crm_fetch,5000);\n"
        "      }\n"
        "    })\n"
        ...
        "      clearTimeout(tid);\n"
        "      console.warn('[CRM]',e);\n"
        "      var ls3=document.getElementById('loadingScreen');\n"
        "      if(ls3)ls3.style.display='none';\n"
        "      if(e.name==='AbortError'){\n"
        "        var le=document.getElementById('loadErr');\n"
        "        if(!le){le=document.createElement('div');le.id='loadErr';le.style.cssText='position:fixed;top:20px;left:50%;transform:translateX(-50%);z-index:10000;max-width:94vw;width:480px;text-align:center;padding:18px 22px;background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.4);border-radius:12px;color:#ef4444;font-size:13px;line-height:1.6';document.body.appendChild(le);}\n"
        "        le.style.display='block';le.innerHTML='&#9203; Tempo esgotado. Tentando novamente...';\n"
        "      }\n"
        "      setTimeout(_crm_fetch,8000);\n"
        "    });\n"
        "}\n"
    ...
    html = html.replace("setInterval(() => carregarDados(), 60000);", "//setInterval desabilitado", 1)
    html = html.replace("setInterval(() => carregarDados(), 30000);", "//setInterval desabilitado", 1)
    html = html.replace("setInterval(carregarDados,120000);", "//setInterval desabilitado", 1)
    html = html.replace("setInterval(carregarDados, 120000);", "//setInterval desabilitado", 1)
...
<!-- Banner de release controlado -->
<div id="releaseBanner" style="display:none;background:#0a1628;border:1px solid #1d3a6e;border-radius:12px;padding:14px 20px;margin-bottom:20px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
  <div style="display:flex;align-items:center;gap:12px">
    <div id="releaseIcon" style="width:38px;height:38px;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0;background:rgba(59,130,246,.15)">🚀</div>
    <div>
      <div id="releaseTitle" style="font-size:13px;font-weight:700;color:#eaeef7">Controle de versão do painel</div>
      <div id="releaseDesc" style="font-size:11.5px;color:#64748b;margin-top:2px">Carregando...</div>
    </div>
  </div>
  <div style="display:flex;align-items:center;gap:10px">
    <div id="releaseStatus" style="font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px"></div>
    <button id="releaseBtn" onclick="liberarParaClientes()"
      style="background:linear-gradient(135deg,#1e3a8a,#3b82f6);border:none;color:#fff;font-size:13px;font-weight:700;
             padding:9px 20px;border-radius:8px;cursor:pointer;font-family:inherit;transition:opacity .15s;display:none">
      🚀 Liberar para clientes
    </button>
    <div id="releaseResult" style="font-size:12px;font-weight:600;display:none"></div>
  </div>
</div>
...
// ── RELEASE CONTROLADO ──────────────────────────────────────────────
function verificarRelease() {{
  fetch('/api/admin/release-status', {{credentials:'include'}})
    .then(r => r.json())
    .then(j => {{
      var banner  = document.getElementById('releaseBanner');
      var icon    = document.getElementById('releaseIcon');
      ...
      if (!j.liberado) {{
        // Nenhuma versão liberada ainda — botão aparece sempre
        icon.textContent = '🚀';
        icon.style.background = 'rgba(59,130,246,.15)';
        title.textContent = 'Controle de versão do painel';
        desc.textContent  = 'Seu painel ainda não foi liberado para os clientes. Teste seu painel e clique em Liberar quando estiver pronto.';
        status.textContent = '⚠ Não liberado';
        status.style.cssText = 'font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;background:rgba(245,158,11,.15);color:#f59e0b;border:1px solid rgba(245,158,11,.3)';
        btn.style.display = 'inline-block';
      }} else if (j.pendente) {{
        // Nova versão disponível mas ainda não liberada
        icon.textContent = '⚡';
        icon.style.background = 'rgba(245,158,11,.15)';
        title.textContent = 'Nova atualização disponível';
        desc.textContent  = 'Você fez deploy de uma nova versão. Teste no "Meu Painel" e libere para os clientes quando estiver pronto. Versão atual dos clientes: ' + (j.stable_data || '—');
        status.textContent = '⚡ Atualização pendente';
        status.style.cssText = 'font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;background:rgba(245,158,11,.15);color:#f59e0b;border:1px solid rgba(245,158,11,.3)';
        btn.style.display = 'inline-block';
      }} else {{
        // Versão mais recente já liberada — tudo OK
        icon.textContent = '✅';
        icon.style.background = 'rgba(34,197,94,.1)';
        title.textContent = 'Painel atualizado';
        desc.textContent  = 'Todos os clientes estão usando a versão mais recente. Liberado em: ' + (j.stable_data || '—');
        status.textContent = '✅ Liberado';
        status.style.cssText = 'font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;background:rgba(34,197,94,.12);color:#22c55e;border:1px solid rgba(34,197,94,.25)';
        btn.style.display = 'none';
      }}
    }})
    .catch(function() {{
      // Se a API falhar, esconde o banner silenciosamente
      document.getElementById('releaseBanner').style.display = 'none';
    }});
}}

function liberarParaClientes() {{
  var btn = document.getElementById('releaseBtn');
  var res = document.getElementById('releaseResult');
  btn.disabled = true;
  btn.textContent = '⏳ Liberando...';
  btn.style.opacity = '.6';
  res.style.display = 'none';

  fetch('/admin/release', {{method:'POST', credentials:'include'}})
    .then(r => r.json())
    .then(j => {{
      btn.disabled = false;
      btn.style.opacity = '1';
      btn.textContent = '🚀 Liberar para clientes';
      res.style.display = 'block';
      if (j.ok) {{
        res.style.color = '#22c55e';
        res.textContent = j.msg || '✅ Liberado!';
        setTimeout(verificarRelease, 1500);
      }} else {{
        res.style.color = '#ef4444';
        res.textContent = '❌ ' + (j.erro || 'Erro ao liberar.');
      }}
    }})
    .catch(function() {{
      btn.disabled = false;
...
// Verifica o status do release ao carregar o admin
verificarRelease();
</script>
</body>
</html>"""
...
"""
    ...
    def render_admin(self, msg=""):
        users = load_users()
        def _esc(s):
            """Escapa caracteres HTML para evitar XSS no painel admin."""
        ...
        if not users:
            ...
            for uname, ud in users.items():
                ...
                parts.append(
                    ...
                    f'<div class="client-name">{_esc(ud.get("nome",""))}</div>'
                    f'<div class="client-user">@{_esc(uname)}</div></div>'
                    f'<span class="status-badge {status_cls}">{status_txt}</span></div>'
                    '<div class="client-info"><b>Kommo:</b> ' + _esc(ud.get('kommo_subdomain','')) + '.kommo.com' +
                    ('<br><b>Email:</b> ' + _esc(ud.get('email','')) if ud.get('email') else '') +
    ...
    def _handle_GET(self):
        ...
        # Status do release — admin consulta para mostrar o botão atualizado
        if path == "/api/admin/release-status":
            if not sess or not sess["is_admin"]: return self.send_json(403, {"erro": "acesso negado"})
            stable_exists = PAINEL_HTML_STABLE.exists()
            preview_exists = PAINEL_HTML.exists()
            stable_mtime  = PAINEL_HTML_STABLE.stat().st_mtime if stable_exists else 0
            preview_mtime = PAINEL_HTML.stat().st_mtime       if preview_exists else 0
            pendente = preview_mtime > stable_mtime if (stable_exists and preview_exists) else preview_exists
            return self.send_json(200, {
                "liberado":       stable_exists,
                "pendente":       pendente,        # nova versão disponível ainda não liberada
                "stable_data":    datetime.fromtimestamp(stable_mtime).strftime("%d/%m/%Y %H:%M") if stable_exists else None,
                "preview_data":   datetime.fromtimestamp(preview_mtime).strftime("%d/%m/%Y %H:%M") if preview_exists else None,
            })
        ...
        if path == "/admin/meu-painel":
            ...
            body = get_painel_html(ud, cache_data=None, is_admin_view=True, preview=True)
            if body: self.send_html(200, body)
            else: self.send_json(500, {"erro": "HTML nao encontrado"})
            return
    ...
    def _handle_POST(self):
        ...
        if path == "/webhook/asaas":
            # Verifica token secreto se configurado (ASAAS_WEBHOOK_TOKEN no ambiente)
            _wh_token = os.environ.get("ASAAS_WEBHOOK_TOKEN", "")
            if _wh_token:
                _recv_token = self.headers.get("asaas-access-token", "")
                if _recv_token != _wh_token:
                    print("[ASAAS webhook] Token inválido — requisição rejeitada")
                    return self.send_json(401, {"erro": "token invalido"})
            try:
                body = self.read_body()
        ...
        if path == "/admin/release":
            if not sess or not sess["is_admin"]: return self.send_json(403, {"erro": "acesso negado"})
            if not PAINEL_HTML.exists():
                return self.send_json(400, {"ok": False, "erro": "Arquivo painel_servidor_v2.html não encontrado no servidor."})
            try:
                conteudo = PAINEL_HTML.read_text(encoding="utf-8")
                tmp = PAINEL_HTML_STABLE.with_suffix(".tmp")
                tmp.write_text(conteudo, encoding="utf-8")
                tmp.replace(PAINEL_HTML_STABLE)
                ts = datetime.now().strftime("%d/%m/%Y às %H:%M")
                print(f"[RELEASE] Painel liberado para todos os clientes em {ts}")
                return self.send_json(200, {"ok": True, "msg": f"✅ Atualização liberada para todos os clientes em {ts}!"})
            except Exception as e:
                print(f"[RELEASE] Erro: {e}")
                return self.send_json(500, {"ok": False, "erro": f"Erro ao liberar: {e}"})
        ...
        if path == "/admin/add":
            ...
            uname = sanitize_username(form.get("username",""))
            if not uname:
...
if __name__ == "__main__":
    print("=" * 52)
    print("  PAINEL CRM v4 - Multi-cliente + Login")
    print("=" * 52)
    print("  Admin : " + CFG['admin_user'] + " / " + ("*" * len(CFG['admin_pass'])))
    print("  URL   : http://localhost:" + str(CFG['porta']))
    print("  Refresh: a cada " + str(CFG['refresh_minutos']) + " min")
    print("=" * 52)
    ...
    if ALERT_EMAIL or GMAIL_USER:
        print("  Alertas: email habilitado -> " + (ALERT_EMAIL or GMAIL_USER))
    else:
        print("  Alertas: configure ALERT_EMAIL e GMAIL_USER/GMAIL_PASS")

    server = ThreadedHTTPServer(("0.0.0.0", CFG["porta"]), Handler)
    print("\n  Servidor rodando em http://0.0.0.0:" + str(CFG["porta"]) + "\n")
    try:
