...
def claude_raiox(stats):
    ...
    for v, s in list(stats.get("vendor_stats", {}).items())[:10]:
        bateu = "✅ BATEU" if s["valor"] >= meta_vend and meta_vend > 0 else "❌ NÃO BATEU"
        vendor_txt += (
            f"  {v}: valor=R${s['valor']:,.0f} | meta_individual=R${meta_vend:,.0f} | {bateu} | "
            f"ganhou={s['ganho']} | perdeu={s['perdido']} | conv={s['taxa_conv']}% | "
            f"parados={s['parados']} | sem_fup={s['sem_fup']}\n"
        )

    prompt = f"""Você é um gestor comercial direto e objetivo. Analise os dados do CRM e retorne um JSON com diagnóstico prático por vendedor.

MÊS: {mes_label} | META TOTAL: R${meta_total:,.0f} | META POR VENDEDOR: R${meta_vend:,.0f}
RESULTADO: R${stats.get('valor_ganho',0):,.0f} ({stats.get('pct_meta',0)}% da meta)
CONVERSÃO GERAL: {stats.get('taxa_conversao',0)}% | LEADS PARADOS: {stats.get('leads_parados',0)} | SEM FOLLOW-UP: {stats.get('leads_sem_fup',0)}
...
{{
  "resultado_geral": {{
    ...
    "numero_bateram": 0,
    "numero_nao_bateram": 0
  }},
  "equipe": [
    {{
      "nome": "nome do vendedor",
      "valor_vendido": "R$ 0.000",
      "meta": "R$ 0.000",
      "pct_meta": 0,
      "bateu_meta": true,
      "pontos_criticos": [],
  ...
  "alertas_operacao": ["alerta 1 direto e específico", "alerta 2"],
  "acoes_prioritarias": ["ação prática 1 com nome do responsável", "ação 2", "ação 3"],
  "conclusao": "2 frases máximo resumindo o mês e o próximo passo mais urgente"
}}

REGRAS OBRIGATÓRIAS:
- Para quem BATEU a meta: pontos_criticos=[] e solucoes=[]
- Para quem NÃO BATEU: liste 2-3 pontos_criticos com números reais (ex: "conv de X% abaixo da média", "Z leads parados sem ação")
- Para cada ponto_critico: 1 solucao prática e específica (ex: "ligar para os 3 leads parados até sexta")
- alertas_operacao: SEMPRE preencha com 2-3 observações relevantes — mesmo que o resultado seja bom, aponte riscos, dependências ou oportunidades de melhoria (ex: "dependência de 1 único vendedor", "X leads parados há mais de 7 dias")
- acoes_prioritarias: SEMPRE preencha com 3-4 ações concretas para o próximo mês, com números e responsáveis quando possível
- Nunca retorne arrays vazios para alertas_operacao ou acoes_prioritarias
- Use apenas português, seja direto e objetivo, sem texto genérico"""
    ...
    try:
        ...
        print(f"[RAIOX] JSON inválido na resposta: {e}")
        raise RuntimeError(f"Resposta inválida da API (não é JSON): {e}")
    except Exception as e:
...
def _html_email_raiox(stats, analise, nome_empresa, mes_label):
    """Gera o corpo HTML do e-mail de Raio-X Comercial."""
    rg  = analise.get("resultado_geral", {})
    eq  = analise.get("equipe", [])
    al  = analise.get("alertas_operacao", [])
    ac  = analise.get("acoes_prioritarias", [])
    con = analise.get("conclusao", "")

    scls = {"acima_da_meta": "#22c55e", "abaixo_da_meta": "#ef4444", "na_meta": "#f59e0b"}.get(rg.get("status",""), "#3b82f6")
    slbl = {"acima_da_meta": "✅ Acima da Meta", "abaixo_da_meta": "❌ Abaixo da Meta", "na_meta": "⚡ Na Meta"}.get(rg.get("status",""), "")

    # Tabela de vendedores
    rows = ""
    for v in eq:
        bateu  = v.get("bateu_meta", False)
        cor    = "#22c55e" if bateu else "#ef4444"
        badge  = "✅ Bateu" if bateu else "❌ Não bateu"
        rows += (
            f"<tr style='border-bottom:1px solid #1d2740'>"
            f"<td style='padding:10px 12px;font-weight:700;color:#eaeef7'>{v.get('nome','')}</td>"
            f"<td style='padding:10px 12px;color:#eaeef7'>{v.get('valor_vendido','—')}</td>"
            f"<td style='padding:10px 12px;color:#8590a8'>{v.get('meta','—')}</td>"
            f"<td style='padding:10px 12px;font-weight:700;color:{cor}'>{v.get('pct_meta',0)}%</td>"
            f"<td style='padding:10px 12px;font-weight:700;color:{cor}'>{badge}</td>"
            f"</tr>"
        )

    # Diagnóstico quem não bateu
    diag = ""
    for v in eq:
        if not v.get("bateu_meta", True) and v.get("pontos_criticos"):
            diag += f"<div style='margin-bottom:14px'><b style='color:#f87171'>{v.get('nome','')}</b><ul style='margin:6px 0 0 0;padding-left:18px'>"
            for i, pc in enumerate(v.get("pontos_criticos", [])):
                sol = (v.get("solucoes") or [])[i] if i < len(v.get("solucoes") or []) else ""
                diag += f"<li style='color:#fca5a5;margin-bottom:4px'>{pc}"
                if sol:
                    diag += f"<br><span style='color:#86efac;font-style:italic'>→ {sol}</span>"
                diag += "</li>"
            diag += "</ul></div>"

    alertas_html = "".join(f"<li style='margin-bottom:6px;color:#fbbf24'>{a}</li>" for a in al)
    acoes_html   = "".join(f"<li style='margin-bottom:6px;color:#93c5fd'><b>{i+1}.</b> {a}</li>" for i, a in enumerate(ac))
...
<div style="max-width:640px;margin:0 auto;padding:32px 16px">
  ...
  <div style="text-align:center;margin-bottom:28px">
    <div style="font-size:22px;font-weight:800;color:#eaeef7">🔍 Raio-X Comercial</div>
    <div style="font-size:13px;color:#8590a8;margin-top:4px">{nome_empresa} · {mes_label}</div>
  </div>

  <div style="background:#0f1320;border:1px solid #1d2740;border-radius:12px;padding:20px 24px;margin-bottom:16px">
    <div style="font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#8590a8;margin-bottom:10px">Resultado Geral</div>
    <span style="display:inline-block;padding:4px 14px;border-radius:20px;font-size:12px;font-weight:700;background:{scls}22;color:{scls};border:1px solid {scls}55">{slbl}</span>
    <div style="display:flex;gap:20px;margin-top:12px">
      <span style="color:#4ade80;font-weight:700;font-size:13px">✅ {rg.get('numero_bateram',0)} bateram</span>
      <span style="color:#f87171;font-weight:700;font-size:13px">❌ {rg.get('numero_nao_bateram',0)} não bateram</span>
    </div>
    {"<p style='color:#c8d0e0;font-size:13px;margin:12px 0 0 0'>"+rg.get('resumo','')+"</p>" if rg.get('resumo') else ""}
  </div>

  <div style="background:#0f1320;border:1px solid #1d2740;border-radius:12px;padding:20px 24px;margin-bottom:16px">
    <div style="font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#8590a8;margin-bottom:14px">Equipe</div>
    <table style="width:100%;border-collapse:collapse">
      <thead><tr style="border-bottom:1px solid #1d2740">
        <th style="text-align:left;padding:6px 12px;font-size:11px;color:#8590a8;font-weight:700">Vendedor</th>
        <th style="text-align:left;padding:6px 12px;font-size:11px;color:#8590a8;font-weight:700">Vendeu</th>
        <th style="text-align:left;padding:6px 12px;font-size:11px;color:#8590a8;font-weight:700">Meta</th>
        <th style="text-align:left;padding:6px 12px;font-size:11px;color:#8590a8;font-weight:700">%</th>
        <th style="text-align:left;padding:6px 12px;font-size:11px;color:#8590a8;font-weight:700">Status</th>
      </tr></thead>
      <tbody>{rows}</tbody>
    </table>
  </div>

  {"<div style='background:#0f1320;border:1px solid #1d2740;border-radius:12px;padding:20px 24px;margin-bottom:16px'><div style='font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#8590a8;margin-bottom:12px'>📋 Diagnóstico</div>" + diag + "</div>" if diag else ""}

  {"<div style='background:#0f1320;border:1px solid #1d2740;border-radius:12px;padding:20px 24px;margin-bottom:16px'><div style='font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#8590a8;margin-bottom:10px'>⚠ Alertas</div><ul style='margin:0;padding-left:18px'>" + alertas_html + "</ul></div>" if al else ""}

  {"<div style='background:#0f1320;border:1px solid #1d2740;border-radius:12px;padding:20px 24px;margin-bottom:16px'><div style='font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#8590a8;margin-bottom:10px'>🎯 Ações Prioritárias</div><ol style='margin:0;padding-left:18px'>" + acoes_html + "</ol></div>" if ac else ""}

  {"<div style='background:linear-gradient(135deg,rgba(59,130,246,.1),rgba(30,58,138,.15));border:1px solid rgba(59,130,246,.3);border-radius:12px;padding:20px 24px;margin-bottom:16px'><div style='font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#93c5fd;margin-bottom:8px'>✅ Conclusão</div><p style='color:#c8d0e0;font-size:13px;margin:0;line-height:1.7'>" + con + "</p></div>" if con else ""}

  <div style="text-align:center;margin-top:24px;font-size:11px;color:#5a657d">
    Raio-X gerado automaticamente pelo Painel CRM · {mes_label}
  </div>
</div>
</body></html>"""
