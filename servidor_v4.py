...
def get_painel_html(user_cfg, cache_data=None, is_admin_view=False, target_user=None):
    ...
    if logo:
        bg_css = (
            ...
            f"body::before{{content:'';position:fixed;inset:0;background:rgba(7,9,15,0.82);z-index:0;pointer-events:none;}}"
            f".container,.header,.card,.tab-content,.funil-bar,.clientes-recorrentes,.vendedores-section,.checklist-section{{position:relative;z-index:1;}}"
            f"</style>"
        )
...
class Handler(BaseHTTPRequestHandler):
    ...
    def parse_multipart(self):
        ...
        environ = {"REQUEST_METHOD": "POST", "CONTENT_TYPE": ctype, "CONTENT_LENGTH": str(clength)}
