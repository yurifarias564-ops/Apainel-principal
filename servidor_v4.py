  ...
  <div class="card" style="margin-top:4px">
    ...
    <div class="meta-wrap">
      ...
      <div style="display:flex;align-items:center;margin-top:6px">
        ...
        <span style="font-size:11px;color:#64748b">da meta de R$ __CMETA__</span>
      </div>
      <div class="meta-labels">
        <span>R$ 0</span><span>R$ __CMETA__</span>
      </div>
    </div>
...
var _tabAtiva = 'hoje';
var COMISSAO_PCT = parseFloat(localStorage.getItem('cor_pct')) || __CPCT__;
var META_COMISSAO = parseFloat(localStorage.getItem('cor_meta')) || __CMETA__;

function switchTab(name) {
  document.querySelectorAll('.tab-content').forEach(el => el.classList.remove('active'));
  document.querySelectorAll('.tab-btn').forEach(el => el.classList.remove('active'));
  document.getElementById('tab-' + name).classList.add('active');
  var tabs = ['hoje','agenda','pipeline','clientes','comissao'];
...
function renderTab(name, d){
  ...
  if(name==='hoje'){
    ...
    var semContato = leads
      .filter(l=>!l.closed_at&&!l.loss_reason_id)
      .map(l=>({nome:l.name||l.nome||'Lead', dias:l.updated_at?Math.floor((Date.now()/1000-l.updated_at)/86400):999}))
      .filter(l=>l.dias>=2)
    ...
    if(!semContato.length){
      ...
      fl.innerHTML = semContato.map(l=>{
        var urgente = l.dias>=5;
        return '<div class="followup-item">' +
          '<div class="followup-dot" style="background:'+(urgente?'#ef4444':'#f59e0b')+'"></div>'+
          '<div style="flex:1"><div class="followup-name">'+escHtml(l.nome)+'</div>'+
          '<div class="followup-dias">Sem contato há '+l.dias+' dia'+(l.dias>1?'s':'')+'</div></div>'+
  ...
  if(name==='comissao'){
    ...
    if(metaInfo)metaInfo.innerHTML='<span class="meta-pct-badge" id="com-meta-pct">'+pctMeta+'%</span><span style="font-size:11px;color:#64748b">da meta de '+fmtBRL(META_COMISSAO)+'</span>';
