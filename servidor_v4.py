...
CORRETOR_PAINEL_HTML = """<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1"/>
...
.header{background:linear-gradient(135deg,__CCOR__,__CCOR2__);padding:20px 24px;display:flex;align-items:center;justify-content:space-between}
.header-title{font-size:18px;font-weight:700}
.header-sub{font-size:12px;opacity:.8;margin-top:2px}
.tab-bar{display:flex;background:#0d1117;border-bottom:1px solid #1d2740}
.tab-btn{flex:1;background:transparent;border:none;border-bottom:2px solid transparent;color:#64748b;font-size:12.5px;font-weight:600;padding:12px 8px;cursor:pointer;font-family:inherit;transition:all .15s}
.tab-btn.active{color:__CCOR__;border-bottom-color:__CCOR__}
.tab-btn:hover:not(.active){color:#eaeef7}
.tab-content{display:none;padding:20px 16px;max-width:680px;margin:0 auto}
.tab-content.active{display:block}
.card{background:#0f1320;border:1px solid #1d2740;border-radius:12px;padding:18px 20px;margin-bottom:14px}
.card-title{font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.07em;margin-bottom:10px}
...
.metric-value{font-size:15px;font-weight:700;color:#eaeef7}
.metric-value.green{color:#22c55e}
.metric-value.yellow{color:#f59e0b}
.metric-value.red{color:#ef4444}
.stat-big{text-align:center;padding:16px 0}
.stat-big .num{font-size:42px;font-weight:800;background:linear-gradient(135deg,__CCOR__,__CCOR2__);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.stat-big .lbl{font-size:12px;color:#64748b;margin-top:4px}
.pipeline-item{display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid #1a2236}
...
.meta-bar{height:8px;background:#1a2236;border-radius:4px;overflow:hidden;margin-top:8px}
.meta-fill{height:100%;border-radius:4px;background:linear-gradient(90deg,__CCOR__,__CCOR2__);transition:width .6s}
.comissao-val{font-size:28px;font-weight:800;color:#22c55e;margin:8px 0}
.comissao-sub{font-size:12px;color:#64748b}
...
<div id="tab-hoje" class="tab-content active">
  <div class="card" style="margin-top:4px">
    ...
    <div class="metric"><span class="metric-label">Sem contato +3 dias</span><span class="metric-value red" id="hoje-parados">&#8212;</span></div>
  </div>
  <div class="card">
    <div class="card-title">Minha atividade</div>
    <div class="metric"><span class="metric-label">Leads trabalhados</span><span class="metric-value" id="at-trabalhados">&#8212;</span></div>
    <div class="metric"><span class="metric-label">Follow-ups pendentes</span><span class="metric-value yellow" id="at-followup">&#8212;</span></div>
    <div class="metric"><span class="metric-label">Neg&#243;cios em andamento</span><span class="metric-value green" id="at-negocios">&#8212;</span></div>
  </div>
</div>
...
<div id="tab-clientes" class="tab-content">
  ...
  <div class="card">
    ...
    <div class="stat-big">
      ...
      <div class="lbl">% de convers&#227;o</div>
    </div>
  </div>
</div>
...
<div id="tab-comissao" class="tab-content">
  <div class="card" style="margin-top:4px">
    <div class="card-title">Estimativa de comiss&#227;o</div>
    <div class="comissao-val" id="com-valor">R$ &#8212;</div>
    <div class="comissao-sub">Baseado em __CPCT__% sobre neg&#243;cios ganhos</div>
    <div style="margin-top:16px">
      <div style="display:flex;justify-content:space-between;font-size:12px;color:#8590a8;margin-bottom:6px">
        <span>Meta mensal</span><span id="com-meta-pct">0%</span>
      </div>
      <div class="meta-bar"><div class="meta-fill" id="com-meta-bar" style="width:0%"></div></div>
      <div style="font-size:11px;color:#5a657d;margin-top:6px">Meta: R$ __CMETA__</div>
    </div>
  </div>
  <div class="card">
    ...
    <div class="metric"><span class="metric-label">Melhor semana</span><span class="metric-value" id="com-semana">&#8212;</span></div>
  </div>
</div>
...
function switchTab(name) {
  ...
  var names = ['hoje','pipeline','clientes','comissao'];
  document.querySelectorAll('.tab-btn')[names.indexOf(name)].classList.add('active');
}
function fmt(n) { return (n === null || n === undefined) ? '—' : n.toString(); }
function fmtMoney(n) { if (!n) return 'R$ 0'; return 'R$ ' + parseFloat(n).toLocaleString('pt-BR',{minimumFractionDigits:0,maximumFractionDigits:0}); }
async function carregarDados() {
  try {
    ...
    const novosHoje = leads.filter(l => { const dt = new Date(l.created_at*1000); dt.setHours(0,0,0,0); return dt.getTime()===hoje.getTime(); }).length;
    document.getElementById('hoje-leads').textContent = fmt(novosHoje);
    const aberto = leads.filter(l => !l.closed_at && !l.loss_reason_id).length;
    document.getElementById('hoje-aberto').textContent = fmt(aberto);
    const convHoje = leads.filter(l => { const dt = l.updated_at ? new Date(l.updated_at*1000) : null; if (!dt) return false; dt.setHours(0,0,0,0); return dt.getTime()===hoje.getTime(); }).length;
    document.getElementById('hoje-conv').textContent = fmt(convHoje);
    const tresDias = Date.now()/1000 - 3*86400;
    const parados = leads.filter(l => !l.closed_at && l.updated_at && l.updated_at < tresDias).length;
    document.getElementById('hoje-parados').textContent = fmt(parados);
    document.getElementById('at-trabalhados').textContent = fmt(convHoje);
    document.getElementById('at-followup').textContent = fmt(parados);
    const negocios = leads.filter(l => !l.closed_at && !l.loss_reason_id && l.sale > 0).length;
    document.getElementById('at-negocios').textContent = fmt(negocios);
    const pipeList = document.getElementById('pipeline-list');
    pipeList.innerHTML = '';
    let totalPipe = 0;
    funis.forEach(f => {
      const cnt = (f.leads||[]).length; totalPipe += cnt;
      const el = document.createElement('div'); el.className = 'pipeline-item';
    ...
    document.getElementById('pipe-total').textContent = fmt(totalPipe);
    const total = leads.length;
    const umMes = Date.now()/1000 - 30*86400;
    const ativos = leads.filter(l => l.updated_at && l.updated_at > umMes).length;
    const ganhos = leads.filter(l => l.closed_at && !l.loss_reason_id).length;
    const perdidos = leads.filter(l => l.loss_reason_id).length;
    document.getElementById('cli-total').textContent = fmt(total);
    document.getElementById('cli-ativos').textContent = fmt(ativos);
    document.getElementById('cli-ganhos').textContent = fmt(ganhos);
    document.getElementById('cli-perdidos').textContent = fmt(perdidos);
    document.getElementById('cli-taxa').textContent = (total > 0 ? Math.round(ganhos/total*100) : 0) + '%';
    const pct = __CPCT__ / 100;
    const meta = __CMETA__;
    const valorTotal = leads.filter(l => l.closed_at && !l.loss_reason_id).reduce((s,l) => s+(l.sale||0), 0);
    const comissao = valorTotal * pct;
    document.getElementById('com-valor').textContent = fmtMoney(comissao);
    document.getElementById('com-ganhos').textContent = fmt(ganhos);
    document.getElementById('com-ticket').textContent = fmtMoney(ganhos > 0 ? valorTotal/ganhos : 0);
    document.getElementById('com-semana').textContent = '—';
    if (meta > 0) {
      const pctMeta = Math.min(100, Math.round(comissao/meta*100));
...
"""
    ...
    def _build_corretores_tab(self):
        ...
        parts.append(
            ...
            '<div class="form-group"><label>Comissao (%)</label><input type="number" name="comissao_pct" placeholder="ex: 3" min="0" max="100" step="0.1" value="3"/></div>' +
            '<div class="form-group"><label>Meta de comissao (R$)</label><input type="number" name="meta_comissao" placeholder="ex: 5000" min="0" step="100" value="5000"/></div>' +
        ...
        if not corretores:
            ...
            for cuname, cd in corretores.items():
                ...
                parts.append(
                    ...
                    f'<br><b>Comissao:</b> {comissao_pct}% &nbsp;|&nbsp; <b>Meta:</b> R$ {meta_comissao}' +
                    '</div>' +
    ...
    def _handle_GET(self):
        ...
        if path.startswith("/corretor/view/"):
            ...
            if not sess["is_admin"] and sess["username"] != cuname:
            ...
            cor1 = cd.get("cor_primaria", "#f59e0b")
            cor2v = cd.get("cor_secundaria", "#d97706")
            cpct = str(cd.get("comissao_pct", 3))
            cmeta = str(cd.get("meta_comissao", 5000))
            page = (CORRETOR_PAINEL_HTML
                .replace("__CNOME__", cd.get("nome", cuname))
                .replace("__CCOR__", cor1)
                .replace("__CCOR2__", cor2v)
                .replace("__CPCT__", cpct)
                .replace("__CMETA__", cmeta)
            )
    ...
    def _handle_POST(self):
        ...
        if path == "/admin/corretor/add":
            ...
            if not pwd:
                ...
                return
            import hashlib
            try: cpct = float(form.get("comissao_pct","3") or 3)
            except: cpct = 3.0
            try: cmeta = float(form.get("meta_comissao","5000") or 5000)
            except: cmeta = 5000.0
            corretores[cuname] = {
