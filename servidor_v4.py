...
def buscar_eventos_chat(base_url, token, desde_dias=60):
    ...
    while True:
        try:
            ...
            data    = kommo_get(req_url, token)
            if not data: break
            items = (data.get("_embedded") or {}).get("events") or []
            for ev in items:
                if (ev.get("entity_type") or "").lower() != "lead": continue
                eventos.append({
            ...
            print(f"  [FU] erro ao buscar eventos pg{page}: {e}")
            break
    print(f"  [FU] {len(eventos)} eventos de mensagem encontrados (últimos {desde_dias}d)")
    return eventos
...
def _buscar_kommo_user_inner(username, subdomain, token):
    ...
    try:
        ...
        # ── Follow-up por sessão WhatsApp ──────────────────────────────────
        try:
            leads_andamento = [l for l in all_leads
            ...
            fu_por_vend  = calcular_fu_por_vendedor(fu_sessoes, leads_andamento)
        except Exception as e:
            print(f"  [FU] erro ao calcular sessoes: {e}")
            fu_sessoes  = {}
            fu_por_vend = {}
