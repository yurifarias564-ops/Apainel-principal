        ...
        if not users:
            ...
            for uname, ud in users.items():
                ...
                parts.append(
                    ...
                    ('<br><b>Link cartao:</b> <a href="' + ud.get('asaas_payment_link','') + '" target="_blank" style="color:#3b82f6">abrir link</a>' if ud.get('asaas_payment_link') else '') +
                    (f'<br><form method="POST" action="/admin/regenerar_link" style="display:inline"><input type="hidden" name="username" value="{uname}"><button type="submit" style="background:none;border:none;color:#f59e0b;cursor:pointer;font-size:12px;padding:0;text-decoration:underline">↻ Regenerar link</button></form>') +
                    '</div>'
