  ...
  if(name==='hoje'){
    ...
    var quentes = leads
      .filter(function(l){
        ...
        var etapa = (l.etapa||l.status||'').toLowerCase();
...
function mostrarErroKommo(msg){
  var msgFinal = msg||'Verifique as credenciais do Kommo no painel do administrador.';
  var errHtml='<div style="text-align:center;padding:14px;color:#ef4444;font-size:12px">⚠️ '+msgFinal+'</div>';
  var fl=document.getElementById('followup-list');
  var ql=document.getElementById('quentes-list');
  if(fl)fl.innerHTML=errHtml;
  if(ql)ql.innerHTML=errHtml;
...
async function carregarDados(){
  try{
    ...
    clearTimeout(timer);
    if(!r.ok){mostrarErroKommo('Erro de conexão com o servidor ('+r.status+').');return;}
    var j=await r.json();
    if(j.status==='erro'||(!j.dados&&j.erro)){
      mostrarErroKommo(j.erro||'Erro ao carregar dados do Kommo.');
      return;
    }
    _dadosCarregados=j.dados||{};
    renderTab(_tabAtiva,_dadosCarregados);
  }catch(e){
    if(e.name==='AbortError'){mostrarErroKommo('Tempo de carregamento esgotado. Recarregue a página.');}
    else{mostrarErroKommo('Erro ao conectar: '+e.message);}
    console.error(e);
  }
}
        ...
        if path == "/api/dados":
            ...
            if c_dados is None and c_status != "atualizando":
                ...
                if cache_key == "__admin__":
                    ...
                    # Sem credenciais: retorna dados vazios imediatamente, sem esperar 90s
                    if not ud2.get("kommo_subdomain") or not ud2.get("kommo_token"):
                        with lock:
                            cache["dados"] = {"leads": [], "funis": [], "status_map": {}}
                            cache["status"] = "ok"
                            cache["atualizado_em"] = datetime.now().isoformat()
                            cache["erro"] = "Credenciais Kommo não configuradas para este corretor."
                    else:
                        threading.Thread(
                            target=buscar_kommo_user,
