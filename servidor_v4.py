    ...
    def do_GET(self):
        ...
        if path == "/painel":
            ...
            if c_dados:
                ...
                # Dados ainda carregando — mostra página de espera com auto-refresh
                nome = ud.get("nome", "Painel CRM")
                cor  = ud.get("cor_primaria", "#3b82f6")
                wait_html = f"""<!DOCTYPE html><html><head><meta charset="UTF-8"/>
<meta http-equiv="refresh" content="5"/>
<title>{nome}</title>
<style>*{{box-sizing:border-box;margin:0;padding:0}}body{{background:#07090f;color:#eaeef7;font-family:-apple-system,sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:16px}}
.spinner{{width:40px;height:40px;border:3px solid #1d2740;border-top-color:{cor};border-radius:50%;animation:spin 1s linear infinite}}
@keyframes spin{{to{{transform:rotate(360deg)}}}}
h2{{font-size:16px;font-weight:600}}p{{font-size:12px;color:#8590a8}}
</style></head><body>
<div class="spinner"></div>
<h2>Carregando seus dados...</h2>
<p>Conectando ao Kommo, aguarde alguns segundos.</p>
<p style="margin-top:8px;font-size:11px;color:#5a657d">Esta página atualiza automaticamente.</p>
</body></html>""".encode("utf-8")
                self.send_html(200, wait_html)
            return
