...
def compute_raiox_stats(dados, mes, meta=50000, vendor_tags=None, filtro_vendedor=''):
    ...
    def _tag_vend(l):
        """Retorna o nome do vendedor a partir das tags do lead."""
        tags_str = l.get("tags") or ""
        if not tags_str:
            return filtro_vendedor or None
        parts = [t.strip().upper() for t in str(tags_str).split(",") if t.strip()]
        # Se há vendor_tags configuradas, usa só essas
        if vendor_tags:
            for t in parts:
                if t in vendor_tags:
                    return t
            return filtro_vendedor or None
        # Auto-detect: ignora followup e tags operacionais
        import re as _re
        for t in parts:
            if len(t) < 2: continue
    ...
    # Stats por vendedor (agrupado por tag, não por conta CRM)
    vendor_stats = {}
    for l in mes_leads:
