...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/painel":
            ...
            with lock:
                c_dados = cache["dados"]
            # Se não tem em memória, carrega do crm_cache.json (fallback)
            if c_dados is None:
                try:
            ...
            # Se ainda não tem, dispara busca e aguarda até 15s
            if c_dados is None:
                t = threading.Thread(
                    target=buscar_kommo_user,
                    args=(uname, ud.get("kommo_subdomain",""), ud.get("kommo_token","")),
                ...
                # Já tem cache — atualiza em background sem bloquear
                threading.Thread(
                    target=buscar_kommo_user,
                    args=(uname, ud.get("kommo_subdomain",""), ud.get("kommo_token","")),
                    daemon=True
