...
def get_painel_html(user_cfg, cache_data=None, is_admin_view=False):
    ...
    head_script = (
        ...
        "  fetch('/api/dados',{signal:ctrl.signal})\n"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/dados":
            ...
            cache_key = "__admin__" if sess["is_admin"] else sess["user"]
            ...
            if c_dados is None and c_status != "atualizando":
                ...
                if sess["is_admin"]:
                    ...
                    ud2 = users.get(sess["user"], {})
                    threading.Thread(
                        target=buscar_kommo_user,
                        args=(sess["user"], ud2.get("kommo_subdomain",""), ud2.get("kommo_token","")),
                        daemon=True
                    ).start()
