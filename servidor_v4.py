    ...
    def do_GET(self):
        ...
        if path == "/admin/meu-painel":
            ...
            if c_dados is None:
                ...
                for _ in range(120):  # aguarda ate 60 segundos
                    time.sleep(0.5)
                    with lock:
                        c_dados = cache["dados"]
                    if c_dados: break
            if c_dados is None:
                espera = ("<html><head><meta charset='utf-8'>"
                          "<meta http-equiv='refresh' content='10'>"
                          "<title>Admin - Carregando</title>"
                          "<style>body{background:#07090f;color:#c8d0e0;font-family:sans-serif;"
                          "display:flex;align-items:center;justify-content:center;min-height:100vh;"
                          "flex-direction:column;gap:16px}"
                          ".sp{width:48px;height:48px;border:4px solid #1e3a8a;"
                          "border-top:4px solid #3b82f6;border-radius:50%;animation:s 1s linear infinite}"
                          "@keyframes s{to{transform:rotate(360deg)}}</style></head>"
                          "<body><div class='sp'></div>"
                          "<h2 style='margin:0'>Carregando dados do CRM...</h2>"
                          "<p style='color:#8590a8;font-size:14px'>Sincronizando com o Kommo. "
                          "A pagina atualiza automaticamente.</p>"
                          "<a href='/admin' style='color:#3b82f6;font-size:13px'>"
                          "Voltar ao Admin</a></body></html>")
                return self.send_html(200, espera.encode("utf-8"))
            body = get_painel_html(ud, cache_data=c_dados, is_admin_view=True)
            if body: self.send_html(200, body)
            else: self.send_json(500, {"erro": "HTML nao encontrado"})
            return
        ...
        if path == "/painel":
            ...
            # Se ainda sem dados: garante que a busca está rodando e AGUARDA até 60s
            if c_dados is None:
                if c_status != "atualizando":
                    threading.Thread(
                        target=buscar_kommo_user,
            ...
            if c_dados is None:
                nome_empresa = ud.get("nome", CFG.get("nome_empresa","CRM"))
                espera = ("<html><head><meta charset='utf-8'>"
                          "<meta http-equiv='refresh' content='10'>"
                          f"<title>{nome_empresa} - Carregando</title>"
                          "<style>body{background:#07090f;color:#c8d0e0;font-family:sans-serif;"
                          "display:flex;align-items:center;justify-content:center;min-height:100vh;"
                          "flex-direction:column;gap:16px}"
                          ".sp{width:48px;height:48px;border:4px solid #1e3a8a;"
                          ...
                          "<h2 style='margin:0'>Carregando dados do CRM...</h2>"
                          "<p style='color:#8590a8;font-size:14px'>Sincronizando com o Kommo. "
                          "A pagina atualiza automaticamente.</p></body></html>")
                return self.send_html(200, espera.encode("utf-8"))
        ...
        if path == "/api/dados":
            ...
            uname = sess["user"]
            if sess["is_admin"]:
                # Admin: retorna dados do cache __admin__
                cache, lock = get_cache("__admin__")
                with lock:
                    resp = {"status": cache["status"], "atualizado_em": cache["atualizado_em"],
                            "erro": cache["erro"], "dados": cache["dados"]}
