...
def get_painel_html(user_cfg, cache_data=None, is_admin_view=False, target_user=None):
    ...
    if is_admin_view:
        back_btn = (
            '<style>'
            '#voltarAdmin{position:fixed;top:14px;right:16px;z-index:9999;background:#1e3a8a;color:#fff;font-size:12px;font-weight:700;padding:7px 14px;border-radius:8px;text-decoration:none;border:1px solid #2c4dbd}'
            '@media(max-width:640px){#voltarAdmin{top:auto;bottom:16px;right:12px;font-size:11px;padding:8px 12px;border-radius:20px;box-shadow:0 4px 16px rgba(0,0,0,.5)}}'
            '</style>'
            '<a id="voltarAdmin" href="/admin">← Admin</a>'
        )
    ...
    head_script = (
        ...
        "        var ls=document.getElementById('loadingScreen');if(ls)ls.style.display='none';\n"
        "        var le=document.getElementById('loadErr');\n"
        "        if(le){le.style.display='block';le.innerHTML='Sem conexão com o servidor.<br><br><button onclick=\'location.reload()\' style=\'background:#1e3a8a;color:#fff;border:none;padding:8px 18px;border-radius:8px;cursor:pointer;font-size:13px;font-weight:700\'>↺ Tentar novamente</button>';}\n"
...
def _load_corretor_html():
    """Carrega painel_corretor.html do disco. Para mudar o corretor, edite esse arquivo."""
    from pathlib import Path
    p = Path(__file__).parent / "painel_corretor.html"
    try:
        return p.read_text(encoding="utf-8")
    except Exception as e:
        print(f"[ERRO] painel_corretor.html nao encontrado: {e}")
        return "<h1>Erro: painel_corretor.html nao encontrado</h1>"
...
"""
  ...
  const d=e.data?e.data.json():{title:'Painel CRM',body:'Nova notificação'};
