...
# DATA_DIR: usa disco persistente do Render se configurado, senão usa pasta local
...
def get_painel_html(user_cfg, cache_data=None, is_admin_view=False, target_user=None):
    ...
    # URL da API: se for admin vendo painel de cliente, passa ?user=X
    api_url = f"/api/dados?user={target_user}" if target_user else "/api/dados"
    ...
    head_script = (
        ...
        f"  fetch('{api_url}',{{signal:ctrl.signal}})\n"
    ...
    # Converte let → var para que window.RAW_DATA e a variavel de closure sejam a mesma
    # (var em script de topo vira propriedade window; let nao)
...
<div class="box">
  ...
  <form method="POST" action="/login">
    <label>Usuario</label>
    <input type="text" name="username" placeholder="seu usuario" autocomplete="username" required/>
    <label>Senha</label>
...
<div class="add-form">
  ...
  <form method="POST" action="/admin/add">
    <div class="form-grid">
      ...
      <div class="form-group">
        <label>Cor primaria (opcional)</label>
...
class Handler(BaseHTTPRequestHandler):
        ...
        if not users:
            ...
            for uname, ud in users.items():
                ...
                parts.append(
                    ...
                    f'<div class="client-info"><b>Kommo:</b> {ud.get("kommo_subdomain","")}.kommo.com</div>'
    ...
    def do_GET(self):
        ...
        if path == "/api/dados":
            ...
            # Admin pode ver dados de qualquer cliente via ?user=X
