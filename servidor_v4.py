...
def load_config():
    ...
    return {
        ...
        "refresh_minutos": int(os.environ.get("REFRESH_MINUTOS") or painel.get("refresh_minutos", 10)),
...
_CANAL_PATTERNS = [
    ("TikTok",    [r"tiktok", r"tik.?tok", r"tt"]),
...
def detect_canal(tags_str, contact_fields, lead_name=""):
    """Detecta o canal de origem do lead de forma precisa.
    Verifica: tags do lead, campos customizados do contato, nome do lead.
    """
    # Texto combinado de todas as fontes de sinal
    signals = []
    if tags_str:
        signals.append(tags_str.lower())
    # Campos customizados do contato: procura campos de origem/utm/source
    for cf in (contact_fields or []):
        fname = (cf.get("field_name") or cf.get("field_code") or "").lower()
        if any(k in fname for k in ("source","utm","origem","canal","midia","mídia","social","rede")):
            for v in (cf.get("values") or []):
                val = str(v.get("value","") or v.get("enum_value","") or "").lower()
                if val:
                    signals.append(val)
    # Nome do lead pode conter dica (ex: "@perfil_tiktok" ou "Lead Facebook")
    if lead_name:
        ...
        for l in leads_raw:
            ...
            for c in contacts:
                ...
                if cid and cid in _contact_cache:
                    ...
                    break
            canal = detect_canal(tags_str, ct_fields, l.get("name",""))
            if canal is None:
