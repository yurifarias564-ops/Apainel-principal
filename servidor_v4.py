...
def compute_raiox_stats(dados, mes, meta=50000, vendor_tags=None, filtro_vendedor=''):
    ...
    # ── FILTRO DE MÊS — aplica em todos os dados ─────────────────────────────
    if mes:
        try:
    ...
    # Métricas operacionais — dentro do mês selecionado
    ...
    # ── STATS POR VENDEDOR (base: leads do mês) ──────────────────────────────
    vendor_stats = {}
    for l in mes_leads:
...
def claude_raiox(stats):
    ...
    for v, s in list(stats.get("vendor_stats", {}).items())[:10]:
        bateu = "BATEU" if s["valor"] >= meta_vend and meta_vend > 0 else "NAO BATEU"
        vendor_txt += (
            f"  {v}: R${s['valor']:,.0f} | meta=R${meta_vend:,.0f} | {bateu} | "
            f"ganhou={s['ganho']} | perdeu={s['perdido']} | andamento={s['andamento']} | "
            f"conv={s['taxa_conv']}% | parados={s['parados']} | sem_fup={s['sem_fup']}\n"
        )
    ...
    prompt = f"""Você é um gestor comercial sênior. Analise os dados do CRM abaixo e gere um relatório executivo em JSON com 7 seções.

{filtro_linha}MÊS: {mes_label}
META TOTAL: R${meta_total:,.0f} | META POR VENDEDOR: R${meta_vend:,.0f}
RESULTADO DO MÊS: R${stats.get('valor_ganho',0):,.0f} ({stats.get('pct_meta',0)}% da meta)
VENDAS NO MÊS: {stats.get('ganhos',0)} | PERDAS NO MÊS: {stats.get('perdidos',0)} | CONVERSÃO DO MÊS: {stats.get('taxa_conversao',0)}%
LEADS RECEBIDOS NO MÊS: {stats.get('total_leads',0)}
PIPELINE ATUAL (todos os leads ativos): {stats.get('em_andamento',0)} leads em negociação
  └ SEM FOLLOW-UP: {stats.get('leads_sem_fup',0)} leads sem nenhuma interação registrada
  └ PARADOS +7 DIAS: {stats.get('leads_parados',0)} leads sem atualização há mais de 7 dias
CANAIS: {canal_txt}
...
REGRAS:
- Use apenas dados reais fornecidos acima — sem inventar números
- equipe.destaques: só preencher se o vendedor tem pontos positivos reais
- equipe.problemas: preencher com dados concretos (ex: "conv de 3.6% — metade da média da equipe", "394 leads sem follow-up")
- equipe.impacto: obrigatório quando há problemas, vazio se não há
- oportunidades: leads parados, etapas com perda, canais subexplorados — use os números reais
- recomendacoes: SEMPRE 3-4 ações práticas e específicas, com responsável quando possível
- conclusao: SEMPRE preencher com resumo executivo claro
- Use apenas português"""
