...
def get_painel_html(user_cfg, cache_data=None, is_admin_view=False):
    ...
    # Sempre embute dados (ou vazio) para evitar loading infinito
...
async function carregarDados(forcar=false){{
  try{{
    if(_DADOS_EMBUTIDOS&&!forcar){{
      ...
      FUNIS=j.dados.funis||[];
      preComputeLeads();renderFunilBar();render(currentRange);
      document.getElementById('loadingScreen').style.display='none';
      if(document.getElementById('loadErr'))document.getElementById('loadErr').style.display='none';
      setPill('live','{pill_txt}');
    }} else {{
      // Long-poll: servidor aguarda até 90s pelos dados
      const ctrl=new AbortController();
      const tid=setTimeout(()=>ctrl.abort(),95000);
      try{{
        const r=await fetch('/api/dados',{{signal:ctrl.signal}});
        clearTimeout(tid);
        ...
        if(j.dados){{
          ...
          document.getElementById('loadingScreen').style.display='none';
          if(document.getElementById('loadErr'))document.getElementById('loadErr').style.display='none';
          setPill('live','Atualizado');
        }} else if(j.erro){{
          console.error('Erro Kommo:',j.erro);
        ...
        clearTimeout(tid);
        setTimeout(()=>carregarDados(),8000);
      }}
    }}
  }}catch(e){{console.error(e);setTimeout(()=>carregarDados(),8000);}}
}}"""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/admin/meu-painel":
            ...
            # Garante que busca está rodando em background
            ...
            # Serve HTML imediatamente — JS vai buscar dados via /api/dados (long-poll)
        ...
        if path == "/painel":
            ...
            with lock:
                c_dados = cache["dados"]
                c_status = cache["status"]
            # Se não tem em memória, tenta carregar do crm_cache.json (cache do disco)
            if c_dados is None:
                try:
                    if CACHE_FILE.exists():
                        c_dados = json.loads(CACHE_FILE.read_text(encoding="utf-8"))
                        with lock:
                            cache["dados"] = c_dados
                            cache["status"] = "ok"
                            c_status = "ok"
        ...
        # API dados do cliente logado (long-poll: aguarda até 90s pelos dados)
        if path == "/api/dados":
            if not sess:
                return self.send_json(200, {"status": "atualizando", "dados": None, "atualizado_em": None, "erro": None})
            uname = sess["user"]
            cache_key = "__admin__" if sess["is_admin"] else uname
            cache, lock = get_cache(cache_key)
            with lock:
                c_dados = cache["dados"]
                c_status = cache["status"]
            # Se não tem dados ainda, garante que busca está rodando e aguarda
            if c_dados is None:
                if c_status != "atualizando":
                    users = load_users()
                    if sess["is_admin"]:
                        ...
                        threading.Thread(
                            target=buscar_kommo_user,
                            args=(uname, ud2.get("kommo_subdomain",""), ud2.get("kommo_token","")),
                            daemon=True
                        ).start()
                for _ in range(180):  # aguarda até 90 segundos
                    time.sleep(0.5)
                    with lock:
                        c_dados = cache["dados"]
                        c_status = cache["status"]
        ...
        # DEBUG: status dos caches (acesso publico para diagnostico)
        if path == "/debug":
            linhas = [f"<h2>Debug — {datetime.now().strftime('%H:%M:%S')}</h2><pre>"]
            linhas.append(f"CFG subdomain: {CFG.get('subdomain','(vazio)')}")
            linhas.append(f"CFG token: {'OK ('+str(len(CFG.get('token','')))+'chars)' if CFG.get('token') else '(VAZIO!)'}")
            linhas.append(f"PAINEL_HTML existe: {PAINEL_HTML.exists()}")
            linhas.append("")
            for uname2, cache2 in _caches.items():
                linhas.append(f"[{uname2}] status={cache2['status']} | leads={len((cache2.get('dados') or {}).get('leads') or [])} | erro={cache2.get('erro')}")
            linhas.append("</pre>")
            body = ("<html><head><meta charset='utf-8'><meta http-equiv='refresh' content='5'>"
                    ...
                    "h2{color:#3b82f6}pre{background:#0f1320;padding:16px;border-radius:8px;line-height:1.7}</style></head>"
                    "<body>" + "".join(linhas) + "</body></html>")
