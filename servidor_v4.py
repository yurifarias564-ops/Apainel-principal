...
def _ultimo_dia_mes(dt):
    """Retorna True se dt é o último dia do mês (cobre meses sem dia 31)."""
    import calendar
...
def auto_raiox_mensal():
    """Thread que roda todo dia e envia o Raio-X no último dia do mês."""
    while True:
        agora = datetime.now()
        # Aguarda até as 08:00 do dia seguinte para checar novamente
        ...
        agora = datetime.now()
        if not _ultimo_dia_mes(agora):
            continue  # Não é o último dia do mês — aguarda próximo ciclo
...
def auto_alertas_email():
    """Thread principal de emails: relatórios às 6h e 19h + alertas a cada 1h."""
    time.sleep(120)  # aguarda 2 min no boot
    _enviado_manha = None  # data do último relatório da manhã
    _enviado_noite = None  # data do último relatório da noite
    while True:
        agora = datetime.now()
        hoje  = agora.date()
        ...
        if agora.hour == 19 and _enviado_noite != hoje:
            print(f"[RELATORIO] Enviando relatório da noite...")
            enviar_relatorio_noite()
            _enviado_noite = hoje

        # Alertas instantâneos (a cada hora, fora dos horários de relatório)
        if agora.minute == 0 and agora.hour not in (6, 19):
...
class Handler(BaseHTTPRequestHandler):
    ...
    def render_admin(self, msg=""):
        ...
        smtp_txt    = f"✅ {GMAIL_USER}" if smtp_ok else "❌ Não configurado (GMAIL_USER / GMAIL_PASS)"
        smtp_bg     = "#0a1f0a" if smtp_ok else "#1f0a0a"
        smtp_border = "#166534" if smtp_ok else "#7f1d1d"

        # Contagem de emails enviados nesta sessão
        with _email_log_lock:
            n_log = len(_email_log)
            n_ok  = sum(1 for e in _email_log if e["status"] == "ok")
            n_err = sum(1 for e in _email_log if e["status"] == "erro")

        # Status de email por cliente
        clientes_email_rows = ""
        for uname, ud in (users.items() if users else {}.items()):
            em = ud.get("raiox_email") or ud.get("email") or ""
            if em:
                clientes_email_rows += (
                    f'<div style="display:flex;justify-content:space-between;align-items:center;'
                    f'padding:6px 0;border-bottom:1px solid #1d274022;font-size:12px">'
                    f'<span style="color:#94a3b8">@{uname} — <b style="color:#cbd5e1">{ud.get("nome","")}</b></span>'
                    f'<span style="color:#60a5fa">{em}</span>'
                    f'</div>'
                )
        if not clientes_email_rows:
            clientes_email_rows = (
                '<p style="font-size:12px;color:#475569;margin:0">Nenhum cliente com email configurado ainda.</p>'
            )
        ...
        email_card = (
            ...
            '<div style="background:#1e3a8a;width:36px;height:36px;border-radius:9px;'
            'display:flex;align-items:center;justify-content:center;font-size:18px">📧</div>'
            '<div><div style="font-size:15px;font-weight:700;color:#eaeef7">Gerenciar Emails</div>'
            '<div style="font-size:11px;color:#64748b;margin-top:1px">Relatórios automáticos, alertas e configuração SMTP</div></div>'
            '</div>'
            '<a href="/admin/emails" style="background:#1e3a8a;color:#fff;font-size:12px;font-weight:700;'
            'padding:8px 16px;border-radius:8px;text-decoration:none;display:inline-flex;align-items:center;gap:5px;'
            'transition:opacity .15s" onmouseover="this.style.opacity=\'.8\'" onmouseout="this.style.opacity=\'1\'">Ver log completo →</a>'
            '</div>'

            # Corpo: status SMTP + contadores
            '<div style="padding:16px 20px;display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;'
            'border-bottom:1px solid #1d2740">'

            # Status SMTP
            f'<div style="background:{smtp_bg};border:1px solid {smtp_border};border-radius:10px;padding:12px 14px">'
            f'<div style="font-size:10px;text-transform:uppercase;letter-spacing:.06em;color:#64748b;margin-bottom:4px">SMTP</div>'
            f'<div style="font-size:12px;font-weight:600;color:{smtp_cor};word-break:break-all">{smtp_txt}</div>'
            '</div>'

            # Emails enviados
            '<div style="background:#0a0f1e;border:1px solid #1d2740;border-radius:10px;padding:12px 14px">'
            '<div style="font-size:10px;text-transform:uppercase;letter-spacing:.06em;color:#64748b;margin-bottom:4px">Enviados (sessão)</div>'
            f'<div style="font-size:22px;font-weight:800;color:#3b82f6">{n_log}'
            f'<span style="font-size:12px;font-weight:400;color:#64748b;margin-left:6px">total</span></div>'
            '</div>'

            # OK / Erro
            '<div style="background:#0a0f1e;border:1px solid #1d2740;border-radius:10px;padding:12px 14px">'
            '<div style="font-size:10px;text-transform:uppercase;letter-spacing:.06em;color:#64748b;margin-bottom:4px">Status</div>'
            f'<div style="font-size:13px;font-weight:700">'
            f'<span style="color:#22c55e">✅ {n_ok}</span>'
            f'<span style="color:#64748b;margin:0 6px">/</span>'
            f'<span style="color:#ef4444">❌ {n_err}</span>'
            '</div>'
            '</div>'

            '</div>'  # end grid

            # Clientes e seus emails
            '<div style="padding:14px 20px">'
            '<div style="font-size:11px;text-transform:uppercase;letter-spacing:.05em;color:#64748b;margin-bottom:8px">Emails por cliente</div>'
            + clientes_email_rows +
            '</div>'

            '</div>'  # end card
        )
        clientes_html = email_card + clientes_html
    ...
    def _render_email_admin(self):
        """Página de gerenciamento de emails — somente admin."""
        with _email_log_lock:
            log_copy = list(_email_log)

        TIPO_LABEL  = {"manha": "☀️ Relatório Manhã", "semana": "📅 Relatório Semana", "alerta": "🔔 Alerta"}
        ...
        for i, e in enumerate(log_copy):
            ...
            assunto_esc = e["assunto"].replace("<","&lt;").replace(">","&gt;")
            dest_esc    = e["destinatario"].replace("<","&lt;").replace(">","&gt;")
            rows += (
                f'<tr onclick="verPreview({i})" style="cursor:pointer" class="erow">'
                f'<td>{ts_fmt}</td>'
                f'<td>{tipo_l}</td>'
                f'<td style="color:#94a3b8;font-size:12px">{dest_esc}</td>'
                f'<td style="max-width:260px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{assunto_esc}</td>'
                f'<td style="color:{cor};font-weight:700">{icon} {e["status"]}</td>'
                f'</tr>'
            )
        if not rows:
            rows = "<tr><td colspan='5' style='text-align:center;color:#475569;padding:32px'>Nenhum email enviado ainda nesta sessão.</td></tr>"

        dest_info = ALERT_EMAIL or GMAIL_USER or "(não configurado)"
        config_ok = bool(GMAIL_USER and GMAIL_PASS)
        cfg_cor   = "#22c55e" if config_ok else "#ef4444"
        cfg_txt   = f"✅ Configurado — {dest_info}" if config_ok else "❌ GMAIL_USER / GMAIL_PASS não definidos"

        html = f"""<!DOCTYPE html>
<html lang="pt-BR"><head>
<meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>Emails — Painel CRM</title>
<style>
*{{box-sizing:border-box;margin:0;padding:0}}
body{{background:#07090f;color:#eaeef7;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;min-height:100vh;padding:28px 24px 60px}}
h1{{font-size:20px;font-weight:700;margin-bottom:4px}}
.sub{{color:#64748b;font-size:12px;margin-bottom:24px}}
.topbar{{display:flex;align-items:center;gap:10px;margin-bottom:24px;flex-wrap:wrap}}
.btn{{padding:8px 16px;border-radius:8px;border:none;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;transition:opacity .15s;color:#fff}}
.btn:hover{{opacity:.85}}
.btn-manha{{background:#1e40af}}
.btn-semana{{background:#0f172a;border:1px solid #1e40af;color:#93c5fd}}
.btn-alerta{{background:#92400e}}
.btn-back{{background:transparent;border:1px solid #1d2740;color:#8590a8;text-decoration:none;display:inline-flex;align-items:center;gap:5px;padding:7px 14px;border-radius:7px;font-size:12px;font-weight:600}}
.btn-back:hover{{color:#fff;border-color:#3b82f6}}
.cfg-bar{{background:#0f1320;border:1px solid #1d2740;border-radius:8px;padding:10px 16px;font-size:12px;color:{cfg_cor};margin-bottom:20px}}
.card{{background:#0f1320;border:1px solid #1d2740;border-radius:12px;overflow:hidden}}
table{{width:100%;border-collapse:collapse;font-size:13px}}
thead th{{padding:10px 14px;text-align:left;font-size:11px;text-transform:uppercase;letter-spacing:.05em;color:#64748b;border-bottom:1px solid #1d2740;background:#0a0f1e}}
tbody td{{padding:10px 14px;border-bottom:1px solid #1d274011;vertical-align:middle}}
.erow:hover{{background:#ffffff08}}
.sending{{opacity:.6;pointer-events:none}}
#toast{{position:fixed;bottom:24px;right:24px;background:#1e3a8a;color:#fff;padding:10px 18px;border-radius:8px;font-size:13px;font-weight:600;display:none;z-index:999}}
.preview-overlay{{display:none;position:fixed;inset:0;background:rgba(0,0,0,.8);z-index:1000;align-items:center;justify-content:center;padding:20px}}
.preview-overlay.open{{display:flex}}
.preview-box{{background:#0f172a;border:1px solid #1d2740;border-radius:12px;width:100%;max-width:640px;max-height:90vh;overflow:hidden;display:flex;flex-direction:column}}
.preview-head{{padding:14px 18px;border-bottom:1px solid #1d2740;display:flex;justify-content:space-between;align-items:center;font-size:13px;font-weight:700}}
.preview-frame{{flex:1;border:none;background:#0f172a;min-height:400px}}
</style>
</head><body>
<div style="max-width:960px;margin:0 auto">
  <a href="/admin" class="btn-back">← Voltar ao Admin</a>
  <h1 style="margin-top:16px">📧 Gerenciamento de Emails</h1>
  <p class="sub">Acompanhe, envie manualmente e visualize os emails automáticos do painel.</p>

  <div class="cfg-bar">Configuração SMTP: {cfg_txt}</div>

  <div class="topbar">
    <button class="btn btn-manha" onclick="enviarManual('manha',this)">☀️ Enviar Relatório de Ontem agora</button>
    <button class="btn btn-semana" onclick="enviarManual('semana',this)">📅 Enviar Relatório da Semana agora</button>
    <button class="btn btn-alerta" onclick="enviarManual('alerta',this)">🔔 Checar e Enviar Alertas agora</button>
    <button class="btn" style="background:#1e293b;margin-left:auto" onclick="location.reload()">↻ Atualizar</button>
  </div>

  <div class="card">
    <table>
      <thead><tr>
        <th>Data / Hora</th><th>Tipo</th><th>Destinatário</th><th>Assunto</th><th>Status</th>
      </tr></thead>
      <tbody id="logBody">{rows}</tbody>
    </table>
  </div>
  <p style="font-size:11px;color:#475569;margin-top:10px;text-align:right">
    {len(log_copy)} email(s) registrado(s) nesta sessão &nbsp;·&nbsp;
    Próximo envio automático: ☀️ 06:00 &nbsp;·&nbsp; 📅 19:00
  </p>
</div>

<div id="toast"></div>

<div class="preview-overlay" id="previewOverlay" onclick="fecharPreview(event)">
  <div class="preview-box">
    <div class="preview-head">
      <span id="previewTitle">Pré-visualização</span>
      <button onclick="fecharPreview()" style="background:none;border:none;color:#94a3b8;cursor:pointer;font-size:18px;font-family:inherit">✕</button>
    </div>
...
function toast(msg, ok=true) {{
  ...
  t.textContent = msg;
  t.style.background = ok ? '#1e3a8a' : '#7f1d1d';
  t.style.display = 'block';
  setTimeout(() => t.style.display = 'none', 3500);
}}

function enviarManual(tipo, btn) {{
  ...
  .then(r => r.json())
  .then(j => {{
    toast(j.msg || j.erro, !!j.ok);
    setTimeout(() => location.reload(), 2000);
  }})
  .catch(() => {{ toast('Erro de conexão', false); btn.classList.remove('sending'); }});
}}

function verPreview(idx) {{
  document.getElementById('previewTitle').textContent = 'Carregando...';
  document.getElementById('previewFrame').src = '/api/admin/emails/preview?idx=' + idx;
  document.getElementById('previewOverlay').classList.add('open');
  // Update title after load
  document.getElementById('previewFrame').onload = function() {{
    document.getElementById('previewTitle').textContent = 'Pré-visualização do Email';
  }};
}}
  ...
  if (!e || e.target === document.getElementById('previewOverlay')) {{
    ...
    def _handle_GET(self):
        ...
        if path == "/admin/emails":
            ...
            self.send_html(200, self._render_email_admin())
            return
    ...
    def _handle_POST(self):
        ...
        if path == "/api/admin/emails/send":
            ...
            if tipo == "manha":
                threading.Thread(target=enviar_relatorio_manha, daemon=True).start()
                self.send_json(200, {"ok": True, "msg": "Relatório de ontem enviando..."})
            elif tipo == "semana":
                threading.Thread(target=enviar_relatorio_noite, daemon=True).start()
                self.send_json(200, {"ok": True, "msg": "Relatório da semana enviando..."})
            elif tipo == "alerta":
                threading.Thread(target=checar_alertas_email, daemon=True).start()
                self.send_json(200, {"ok": True, "msg": "Alertas verificando e enviando..."})
            else:
                self.send_json(400, {"erro": "tipo invalido. Use: manha, semana, alerta"})
            return
        ...
        if path == "/admin/add":
            if not sess or not sess["is_admin"]: return self.redirect("/login")
            # Form usa multipart/form-data (tem campo de arquivo)
            ...
            if not uname:
                ...
                return
            users = load_users()
            # Salvar imagem de fundo se enviada
            logo_url = ""
